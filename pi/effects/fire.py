...
class FireEffect(LEDEffect):
    ...
    def __init__(self, device: LEDDevice, colors: ColorRamp, height: float, flare_chance: float, flare_brightness: float):
    ...
    def update(self, game_info_table: NetworkTable):
        ...
        for i in range(self._len):
            self.get_device().get_neopixel()[i] = self._colors.get_color(self._buf[i]).to_tuple()
...
def parse(device: LEDDevice, data):
    ...
    return FireEffect(device, colors, height, flare_chance, flare_brightness)
