...
class ConwayEffect(LEDEffect):
    ...
    def __init__(self, segment: LEDSegment, color: Color, fullness: float = 0.5, initial_pattern: list[list[int]] | None = None):
        ...
        if initial_pattern is not None:
            ...
            self._buf = [[1 if random.random() > fullness else 0 for _ in range(self._segment.get_width())] for _ in range(self._segment.get_width())]

    def start(self):
        w, h = len(self._buf[0]), len(self._buf)
        for y in range(h):
            for x in range(w):
                self._segment.get_buffer().pixel(x, y, self._color.to_hex_int() if self._buf[y][x] else 0x000000)
        self._segment.get_buffer().display()
    ...
    def update(self, game_info_table: NetworkTable):
        ...
        for y in range(h):
            for x in range(w):
                ...
                self._segment.get_buffer().pixel(x, y, self._color.to_hex_int() if new_buf[y][x] else 0x000000)
        self._segment.get_buffer().display()
...
def parse(segment: LEDSegment, data):
    ...
    return ConwayEffect(segment, color, fullness=fullness, initial_pattern=initial_pattern)
